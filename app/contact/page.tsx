...
function LampBot() {
  ...
  useEffect(() => {
    ...
    // ── Lights ──
    // Very dim ambient so the robot is barely visible without spotlight
    const ambient = new THREE.AmbientLight(0xffffff, 0.1);
    scene.add(ambient);

    // Always-on soft rim from behind-top (gives the robot shape)
    const rim = new THREE.DirectionalLight(0xffffff, 0.55);
    rim.position.set(-3, 5, -4);
    scene.add(rim);

    // Small fill so robot body isn't totally black
    const fill = new THREE.DirectionalLight(0xffffff, 0.3);
    fill.position.set(2, 2, 3);
    scene.add(fill);

    // ── SPOTLIGHT — lives outside robot group, aimed at cursor ──
    const spot = new THREE.SpotLight(0xffffff, 120, 30, Math.PI / 8, 0.32, 1.4);
    spot.castShadow = true;
    spot.shadow.mapSize.set(2048, 2048);
    spot.shadow.camera.near = 0.5;
    spot.shadow.camera.far = 30;
    // Target must be in scene
    ...
    const dimGl = new THREE.MeshStandardMaterial({
      color: 0x999999,
      emissive: 0xffffff,
    ...
    // ── Robot root (whole body rotates to follow cursor) ──
    ...
    const base = new THREE.Mesh(
      new THREE.CylinderGeometry(0.52, 0.65, 0.16, 32),
      dark,
    );
    base.position.y = -2.5;
    ...
    const baseRing = new THREE.Mesh(
      new THREE.TorusGeometry(0.535, 0.022, 8, 40),
      dimGl,
    );
    ...
    const stemMesh = new THREE.Mesh(
      new THREE.CylinderGeometry(0.09, 0.13, 1.6, 16),
      mid,
    );
    ...
    for (const y of [-0.88, -1.46]) {
      const r = new THREE.Mesh(
        new THREE.TorusGeometry(0.125, 0.02, 8, 24),
        dark,
      );
    ...
    const headMesh = new THREE.Mesh(
      new THREE.BoxGeometry(1.52, 0.92, 0.86),
      white,
    );
    ...
    const visorMesh = new THREE.Mesh(
      new THREE.BoxGeometry(1.18, 0.36, 0.065),
      visorM,
    );
    ...
    for (const y of [0.21, -0.158]) {
      const line = new THREE.Mesh(
        new THREE.BoxGeometry(1.18, 0.01, 0.065),
        dimGl,
      );
    ...
    // LAMP LENS on top-front of head
    const lensBody = new THREE.Mesh(
      new THREE.CylinderGeometry(0.2, 0.33, 0.26, 32, 1, true),
      lensM,
    );
    ...
    const lensRing = new THREE.Mesh(
      new THREE.TorusGeometry(0.33, 0.02, 10, 40),
      dimGl,
    );
    ...
    // The lens has its own tiny fill light
    ...
    const port = new THREE.Mesh(
      new THREE.CylinderGeometry(0.036, 0.036, 0.034, 12),
      dark,
    );
    ...
    // ── Floor (receives shadows from spotlight) ──
    ...
    // Helpers for world-space head position
    const headWorldPos = new THREE.Vector3();
    const headWorldDir = new THREE.Vector3();
    ...
    function animate() {
      ...
      // Smooth lerp entire body toward cursor
      ...
      // Get the world-space forward direction of the head
      // The lens points in local +Z of headGroup, rotated by root
      headGroup.getWorldQuaternion(worldQuat);
      headWorldDir.set(0, 0.35, 1).applyQuaternion(worldQuat).normalize();

      // Place spotlight at the lens position in world space
      const lensWorldPos = headWorldPos
        .clone()
        .add(
          new THREE.Vector3(
            0,
      ...
      spot.position.copy(lensWorldPos);

      // Target = lens position + forward direction * distance
      const targetDist = 8;
      spotTarget.position
        .copy(lensWorldPos)
        .addScaledVector(headWorldDir, targetDist);
...
function GlassButton({ loading }: { loading: boolean }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        transition:
          "color 0.3s cubic-bezier(0.22, 1, 0.36, 1), transform 0.25s cubic-bezier(0.22, 1, 0.36, 1)",
        transform: active
    ...
    >
      ...
      <span
        style={{
          ...
          background: `linear-gradient(180deg, rgba(255,255,255,${hover ? 0.26 : 0.16}) 0%, transparent 100%)`,
          pointerEvents: "none",
      ...
      <span
        style={{
          ...
          inset: 0,
          borderRadius: "inherit",
          border: `1px solid rgba(255,255,255,${hover ? 0.3 : 0.13})`,
  ...
  style={{
    ...
    boxShadow: hover
      ? "0 10px 30px rgba(255,255,255,0.08), 0 0 20px rgba(255,255,255,0.06)"
      : "none",
    pointerEvents: "none",
  }}
/>
...
function ContactForm() {
  ...
  return (
    ...
    >
      ...
      {/* <Toast show={toast} message="Message sent successfully." /> */}
    </div>
  );
}
      ...
      >
        ...
        <div
          style={{
            ...
            width: 1,
            background:
              "linear-gradient(to bottom, transparent, rgba(255,255,255,0.06) 25%, rgba(255,255,255,0.06) 75%, transparent)",
            pointerEvents: "none",
            zIndex: 1,
          }}
        />
